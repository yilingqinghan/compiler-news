...
from pipelines.taxonomy import classify  # ← 新增
...
def clean_text(html, url):
    ...
    try:
        doc = Document(html); txt = doc.summary(html_partial=False)
        if txt and len(txt) > 300: return txt
    except Exception: pass
    try:
        g = Goose(); art = g.extract(raw_html=html)
        if art and art.cleaned_text and len(art.cleaned_text) > 300: return art.cleaned_text
    except Exception: pass
    return None
...
def main():
    ...
    for id_, source, url, title, ts, raw_html in rows:
        ...
        text = clean_text(raw_html, url) if (raw_html and len(raw_html) > 100) else ""
        meta = {"source": source}

        # ★ 分类打标签
        tags = classify(title or "", text or "", url or "", source or "")
        meta.update(tags)
